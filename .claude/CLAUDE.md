...
### ESLint へ再発防止装置を足すとき（重要）

> [!IMPORTANT]
> **flat config は、同じ規則名を持つ後続ブロックで設定を「丸ごと」置き換える。マージしない。**
> `files:` で対象を絞っても、それはファイル選択の話であって規則設定の合成の話ではない。
> **同じ規則名を2箇所で使った瞬間に、先に書いたほうが黙って死ぬ。**
>
> 2026-09-20（#253）に `eslint --print-config` で確認したところ、次の2つが**一度も効いていなかった**。
>
> | 消えていたガード                                      | 消していた後続ブロック    |
> | ----------------------------------------------------- | ------------------------- |
> | #156 `EVENTS_FALLBACK_TREE` の `useSearchParams` 禁止 | #237 の `next/image` 禁止 |
> | #230 の禁止色（`EventInfiniteList.tsx` でのみ）       | #239 の `hasMore` 禁止    |
>
> `no-restricted-imports` と `no-restricted-syntax` は汎用名なので衝突しやすい。
> 現在は `RESTRICTED_IMAGE_IMPORTS` と `RESTRICTED_COLOR_SELECTORS` を
> `eslint.config.mjs` 冒頭へ括り出してある。**この2つの規則名を新しいブロックで使うときは、
> 必ずその定義を展開して合成し、広い側（`src/**`）より後ろへ置くこと。\*\*

**ガードを足したら、退行を注入して `pnpm lint` が exit 1 になることまで実測する。**
「規則を書いた」は装置ではない。`--print-config` で自分の設定が生き残っているかを見るのが最短。
実測の記録は [`docs/dev/testing.md`](../docs/dev/testing.md) と各機能のドキュメントへ残す。
