...
| `NEXT_PUBLIC_SPONCERS_VISIBLE`      | 協賛企業バナーの公開フラグ         | `false`                   | **登録済み（`true`）**      |
...
> [!NOTE]
> **この表は「登録すべきもの」と現在の登録状況を併記している。** `gh variable list` で確認できるのは `NEXT_PUBLIC_URL`、`NEXT_PUBLIC_SPONCERS_VISIBLE`、`NEXT_PUBLIC_SPECIAL_GOODS_VISIBLE`（2026-08-25 実測）。**`EVENTS` / `NEWS` / `SPECIAL` の3フラグは GitHub 側が未登録のまま。**
...
- name: Build project
  ...
  env:
    ...
    NEXT_PUBLIC_SPONCERS_VISIBLE: ${{ vars.NEXT_PUBLIC_SPONCERS_VISIBLE }}
...
| `vars.NEXT_PUBLIC_SPONCERS_VISIBLE`      | `NEXT_PUBLIC_SPONCERS_VISIBLE=false`      |
...
- `NEXT_PUBLIC_SPONCERS_VISIBLE=false`: トップページと多言語 About ページの協賛企業バナーコンポーネントを描画せず、microCMS の協賛企業データも取得しない。`/about/sponsors` の一覧ページは対象外。
...
grep -c '^NEXT_PUBLIC_.*_VISIBLE=' .env.example
...
| `NEXT_PUBLIC_SPONCERS_VISIBLE`      | Production=`false`、Preview=`true`                   |
