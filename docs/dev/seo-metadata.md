...
**`notFound()` は現在 HTTP 404 を返す**（2026-09-19 実測。`/events/` `/special/` `/info/` の
存在しないIDはいずれも 404）。

かつてはそうではなかった。ルート直下の `src/app/loading.tsx` がストリーミングのシェルを
先に送出するため `notFound()` がステータスへ反映されず、2026-09-03 の実測では
`/events/__no_such_id__` が 200 を返して自分自身を canonical に指定していた。
任意の文字列で薄いURLを無限に生成できる状態だった。**この `loading.tsx` は #217
（`417e3a9`）で削除済みだが、ルート直下へ戻すとソフト404が再発する。**

ステータスが正しくなった後も、次の対策は外さない。**404 本文そのものの品質は別問題**である。
