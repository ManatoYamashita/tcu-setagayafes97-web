...
| 退行させた内容                                  | 落ちたテスト                                                                           |
| ----------------------------------------------- | -------------------------------------------------------------------------------------- |
| `paginateEvents` の1未満クランプを削除          | 1未満のページは空配列を返す／小数のページを切り捨てる（**#239 で撤去。契約ごと反転**） |
| `Math.floor` による小数の切り捨てをやめる       | 小数のページを切り捨てる                                                               |
| `filterEvents` の `date` 判定から `both` を外す | 1日目・2日目に両日開催を含める（#207 で反転。上表は当時の記録）                        |
| `generatePageNumbers` の窓幅を 7 から 5 へ      | 上記4つの不変条件を検査するプロパティテストを含む6件（**#239 で撤去**）                |

> [!IMPORTANT]
> **`paginateEvents` / `getTotalPages` / `generatePageNumbers` は #239 で撤去された。**
> ページ分割をやめて無限スクロールへ移したため、上表1行目の「1未満のページは空配列を返す」は
> `resolveVisibleCount` で**反転している**。範囲外を空にすると `?page=-1` や `?page=999` が
> カードの無い真っ白な一覧になるため、いまは常に最低1ページ分を返し総件数へ丸める。
> **上表は当時の記録として残してあり、現在の不変条件は下表である。**

2026-09-20 に `resolveVisibleCount` へ注入しなおした（#239）。

| 退行させた内容                            | 落ちたテスト                                                                                       |
| ----------------------------------------- | -------------------------------------------------------------------------------------------------- |
| 1未満のページを空（0件）へ戻す            | 1未満のページでも1ページ分は返す／どんなページ番号でも「総件数以下」かつ「1件以上」に収まる（2件） |
| 総件数への丸め（`Math.min`）を外す        | 総件数を超える指定は総件数へ丸める／該当0件なら0を返す／上記プロパティテスト（3件）                |
| `Math.floor` による小数の切り捨てをやめる | 小数のページを切り捨てる（1件）                                                                    |
