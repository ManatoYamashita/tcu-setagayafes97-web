...
正規化後も、microCMS 側の未入力値は空文字として残る。企画一覧と詳細は
`src/lib/event-display.ts` の表示用フォールバックを使い、タイトル・主催・概要・会場を
「準備中」「未定」として表示する。画像がない場合もカードと詳細ページに専用プレースホルダーを
表示し、画像の有無でレイアウトが崩れないようにする。

これは欠損データを自動補完する仕組みではない。入稿内容の修正は引き続き microCMS で行い、
公開画面では不完全なデータをエラーや空欄にせず案内するための防御である。
