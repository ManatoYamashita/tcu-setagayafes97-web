...
### 公開サイトの API キー権限と 404

2026-09-18、`news` の下書きが本番に表示された。API キーに「下書きコンテンツの全取得」と
「公開終了コンテンツの全取得」が付いていたため、キャッシュを消すだけでは再生成時に再び取得された。
公開サイトで使うキーから両権限を外し、キャッシュ削除と本番再ビルド後に一覧・サイトマップから
下書きが消えたことを確認した。現在のプランは API キーを1個しか作れず、別キーによる分離はできない。

`/info/[id]` と `/events/[id]` は存在しないコンテンツで `notFound()` を呼ぶ。ルートや
動的詳細ルートの `loading.tsx` が先にレスポンスをストリーミングすると、404画面と
`noindex` が出ても HTTP ステータスは200になる。HTTP 404を維持するため、これらのルートには
`loading.tsx` を置かない。変更時は開発サーバではなく `pnpm build` → `pnpm start` または本番で、
下書き・削除済みURLのステータスを確認する。

### 手で控えた draftKey は使えない

**`draftKey` は保存のたびに変わり、失効する。** 控えたキーを curl に貼って確認しようとすると、
次の保存で無効になり 404 で止まる。
...
**これは draftKey が使えないという意味ではなく、手で運ぶ運用が成立しないという意味である。**
microCMS の「画面プレビュー」ボタンは押すたびに**その時点で有効な** draftKey を URL へ埋めるため、
失効の問題を受けない。受け口は `/api/draft` にあり、本番と同じ詳細ページで下書きを表示する
（設定手順と設計は [draft-preview.md](./draft-preview.md)）。

上の「下書きが本番に表示された」事故とは別の経路である点に注意すること。
事故は**誰にでも**見える状態だったが、画面プレビューはシークレットと有効な draftKey を
持つ者だけが単一のコンテンツを見る。API キーの「下書きコンテンツの全取得」権限は
**外したままでよい**（単一コンテンツの draftKey 取得はこの権限を必要としない）。

### 確認方法の使い分け

| 目的                       | 方法                                                                                                                                     |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **入稿内容の確認**         | microCMS の**画面プレビュー**。本番と同じ実装・同じ見た目で、実機（スマホ）からも確認できる                                              |
| **表示・レイアウトの確認** | コンポーネントへダミーを直接渡す一時ページを作る。CMS に依存せず、**未入力フィールドの挙動まで確認できる**。確認後に削除しコミットしない |

> [!WARNING]
> **かつてここにあった「一時的に公開して確認する」は使えない。**
> 「公開フラグ（`NEXT_PUBLIC_*_VISIBLE`）が false なら本番に出ない」という前提で成立していたが、
> 4フラグすべてが `true` になった現在、公開すればそのまま本番に出る。
