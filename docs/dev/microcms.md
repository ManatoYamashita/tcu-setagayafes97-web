...
`/info/[id]` は存在しない記事で `notFound()` を呼ぶ。ルートの `loading.tsx` が先に
レスポンスをストリーミングすると、404画面と `noindex` が出ても HTTP ステータスは200になる。
HTTP 404を維持するため、ルート共通の `loading.tsx` は置かない。変更時は開発サーバではなく
`pnpm build` → `pnpm start` または本番で、下書き・削除済みURLのステータスを確認する。
