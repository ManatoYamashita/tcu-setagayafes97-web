...
- キャンパスマップと交通アクセス情報による会場案内を実現する（3Dマップは第97回では見送り。2026-08-09 決定）
...
| 項目     | 仕様                                                                              |
| -------- | --------------------------------------------------------------------------------- |
| 形式     | GSAPタイムライン（4フェーズ、合計約1.12秒）                                       |
| 表示条件 | デスクトップ（≥768px）かつ `prefers-reduced-motion` 未指定のときのみ、毎回        |
| 演出     | 白アイコンのフェードイン → カラーへクロスフェード + 背景を濃紫へ → スライドアウト |
| スキップ | なし（尺が約1.1秒のため不要と判断）                                               |
| 実装     | `src/components/layout/{OpenerLoader,Opener}.tsx` + `src/lib/motion.ts`           |

モバイルでは LCP 対象のヒーロー画像を覆わないよう、`OpenerLoader` が GSAP チャンクごと
読み込まない。0.52秒地点で `opener-done` を発火し、各ページの入場アニメーションはこれを待つ。
...
- 詳細説明（Markdown/HTML）
- SNSリンク（任意）
- ~~「マップで見る」ボタン（3Dマップへの遷移）~~ — 3Dマップ見送りにより不要
...
> [!IMPORTANT]
> **第97回では 3D キャンパスマップを実装しません（2026-08-09 決定）。** 本節末尾のリスク管理にある「実装難易度が高い場合は 2D マップをフォールバックとして用意」を発動し、2D で確定しました。判断の詳細は [todo.md](./todo.md) の Phase 3「3Dマップ実装 — 見送り」を参照してください。

**採用する仕様（2Dマップ）**

| 項目     | 仕様                                                            |
| -------- | --------------------------------------------------------------- |
| 実装技術 | 静的画像 + Google Maps 埋め込み                                 |
| 実装箇所 | アクセスページ（`src/components/access/AccessPageContent.tsx`） |
| 内容     | キャンパスマップ画像、最寄り駅からのルート、駐車場・駐輪場情報  |

**見送った仕様（3Dキャンパスマップ）** — 第98回以降で再検討する場合の参考として残します。
...
想定していたクリック後の動作は、建物クリック → その建物内の企画一覧をオーバーレイ表示 → 企画クリックで詳細ページへ遷移、というものでした。
...
- ~~3Dマップ実装~~ — 見送り（2026-08-09）。2Dマップで確定
- [ ] アクセスページ
- [ ] インフォメーション各ページ
- [ ] お問い合わせフォーム
...
- ~~3Dモデル作成ツール（Blender等）~~ — 3Dマップ見送りにより不要
- [ ] ロゴアニメーション素材の納品時期
- [ ] 各企画の情報
