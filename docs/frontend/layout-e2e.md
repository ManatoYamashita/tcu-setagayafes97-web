...
14の通常ルートと4つの404画面を回り、`main#content` がちょうど1つであることを数えます。
代表2ルートでは、スキップリンクがタブ順の先頭であることと、押すとフォーカスが
...
> [!IMPORTANT]
> **`locator.count()` で数えないこと。** 一度きりの評価なので、ストリーミングで後から
> 差し込まれる404画面では 0 を掴みます。`/special/<存在しないID>` で実際に踏みました
> （失敗時のスナップショットにはリンクが写っていた＝単なる早すぎる評価）。
> `toHaveCount` / `expect.poll` は expect のタイムアウトまで自動で再試行します。

退行注入で装置の実効性を確認済みです（2026-09-19）。`src/app/not-found.tsx` の
`<main>` を `<div>` へ戻すと、それを共有する3ルートとフォーカス検査が落ちます。
