...
閾値は「その密度の内容が**余白ごと**収まるカード実寸」です（`full` 90.5px / `compact` 43.5px）。
...
15px のタイトルと13pxの補足情報を余白ごと収めるためです。`py-3` に戻すと閾値も連動して
...
`/timetable` はブランド紹介よりも「当日の予定を確認する」ことが主目的です。
共通 `PageHero` の既定 `70svh` は他ページのまま維持し、このページだけ `compact` を指定します。

- モバイル: `52svh`、最低 `360px`
- `lg` 以上: `50svh`

これにより対応下限の 320×640px でも、日程選択の全体が初期ビューポート内へ入ります。
共通ヒーローの既定値そのものを縮めると他ページのブランド表現まで変わるため、
`PageSheetLayout` の `heroSize` から明示的に選ぶ設計にしています。
...
全ステージの企画を開始時刻順に並べる縦スタック（`TimetableStackedList`）へ切り替えます。
`md`(768px) ではありません。各カードにステージ名を明示するため、ステージ間を往復せず
「次に始まる企画」を比較できます。ステージで絞り込んだ場合も同じ順序を維持します。

`PageSheetLayout` が既にモバイルの左右余白を持つため、`TimetableContent` と
`TimetableChart` は `sm` 未満で左右パディングを重ねません。320px幅でカード本文を
250px以上確保し、余白の入れ子だけで本文幅を失わないことをE2Eで固定します。

一覧はカード幅を削らず、`<ol>` の外側余白に連続した時刻レールを描きます。
各カードでは開始時刻を大きく、終了時刻を小さく置き、縦に視線を動かすだけで
次の開始時刻を比較できる文字階層にします。レールと点は疑似要素なので読み上げには混ざらず、
順序と時刻は `<ol>` と `<time>` の実要素が伝えます。

ガント盤面の左アクセント線は時間枠の識別に必要ですが、モバイルでは外側の時刻レールが
同じ役割を担うため外します。2本の紫線を並べず、記憶点を1つに絞るためです。
...
| カードの高さと密度            | `getCardDensity(94.4) === "compact"`（**枠ではなくカード実寸**で判定）       |
...
| 日程選択が初期画面内 | `responsive-parity.spec.ts`  |
