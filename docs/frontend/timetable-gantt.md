...
「その他」は `stages` 配列には足していません。この配列はキャンパスマップや企画一覧からも
参照されるため、実在しない会場を混ぜると他所へ漏れます。`OTHER_STAGE_ID` は独立した定数です。
...
| 企画長 | 高さ   | 密度      | 出す情報               |
| ------ | ------ | --------- | ---------------------- |
| 15分   | 28px\* | `minimal` | タイトルのみ           |
| 30分   | 48px   | `compact` | タイトル + 時刻        |
| 60分   | 96px   | `full`    | タイトル + 時刻 + 場所 |

\* 本来 24px ですが `MIN_EVENT_HEIGHT_PX = 28` にクランプします。カード内側の余白 4px を
引いた実寸が 24px となり、WCAG 2.5.8（ターゲットサイズ AA）の 24×24 を満たします。
...
テストランナーは未導入のため、ビルドと実ブラウザでの実測で確認します。
