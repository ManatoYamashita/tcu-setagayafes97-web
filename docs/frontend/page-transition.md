...
#### 多重登録を「フラグで抑止」してはいけない

静的解析ツールはこのトップレベル登録を見て「HMR でモジュールが再評価されるとリスナが重複登録
される。`window` にフラグを置いて防げ」と指摘してきます（PR #59 の Copilot レビューで実際に来ました）。
**この指摘に従うと、開発時に履歴遷移アニメーションが完全に死にます。**

Fast Refresh がモジュールを再評価すると、**新しいモジュールインスタンスは新しい `record`
バインディングを持ちます。** `PageTransitionWrapper` が読むのは新インスタンスの `record` です。
ここでフラグによって新インスタンスの登録をスキップすると、書き込むのは古いインスタンスの
リスナ（＝古い `record`）だけになり、**読み側の `record` は永久に `null` のまま**になります。
症状は「ホットリロード後、戻る・進むが無演出になる。フルリロードすると直る」という、
原因を追いにくい壊れ方です。

重複登録そのものは無害です。ハンドラが**冪等**だからです。
...
カウンタも配列 push も無く、2回走っても同値を書くだけです。`pageshow` の `record = null` も同様
です。古いインスタンスのリスナは自分の孤立した `record` へ書くだけで、読み側からは見えません。

production では単一バンドル・単一評価なので重複自体が起きません。webpack / Turbopack の
モジュールレジストリは module id でインスタンスを一意化するため、「複数チャンクに入ったら重複する」
という懸念も成立しません（module federation のように独立した runtime が同一ページに複数載る場合を
除きます。本プロジェクトはそうではありません）。

残るのは開発時に数個のクロージャが `window` に積まれることだけで、リロード回数に比例する有界な量
です。**現在の実装が正しく、変更は不要です。**
