...
**しかもこのずれは広い画面では消える。** `max-w-7xl` が両方で効く幅
（`vw ≥ 1344px`）では、どちらも「1280px を中央寄せ」になって偶然一致するためである。
実測（2026-09-05）:
...
**1440px だけ見て「揃った」と判断すると取り零す。** 1280〜1366px の一般的なノートPCは
ずれる側にいる。修正は外側の節にも `mx-4 sm:mx-6 lg:mx-8` を持たせること。
...
> [!WARNING]
> **解除に `lg:rounded-2xl` と書いてはいけない。** TailwindCSS v4 はクラス文字列の順ではなく
> **プロパティ順**で出力するため、一括指定の `rounded-*` は長形式の `rounded-b-none` より
> **前**に置かれ、変種（`lg:`）であっても負ける。`lg:rounded-b-2xl` と、**同じ長形式ペアで
> 上書きする**こと。同じ罠が `border-*` と `border-l-*` にもある。
> 検査はクラスの有無ではなく算出値で行う。
>
> ```js
> getComputedStyle(el).borderBottomLeftRadius; // "16px" であること。"0px" なら負けている
> ```
