...
**もう一つの解は、器のほうを中身に合わせることである。** トラックの幅を固定サイズ要素に
合わせてしまえば、その要素はトラックを埋めるようになり、左端は何もしなくても器と一致する。
`/events/[id]` のアイコン分岐は最終的にこちらを採った（`lg:grid-cols-[18rem_minmax(0,1fr)]`。
#190）。**中身を左端へ寄せるのは誤りだが、器を中身に合わせるのは正しい。**
副次的に、残りを受け取る列（ここでは見出し列）が 667px から 880px へ広がる。
