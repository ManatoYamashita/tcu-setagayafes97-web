...
`<html>` を持つ `src/app/layout.tsx` はロケールを受け取らず、ロケールを知る
`src/app/[locale]/layout.tsx` は `<html>` を持たない。そのため `<html lang>` は
`"ja"` 固定になり、`/en/access` でも英語の本文が日本語の音声エンジンで読み上げられる。

### 却下した案: `headers()` から `X-NEXT-INTL-LOCALE` を読む

**ルートレイアウトで `headers()` を呼ぶとサイト全体が動的レンダリングへ落ちる。**
実測では、トップ・企画・タイムテーブル・お知らせを含む**ほぼ全ページが
`○ (Static)` から `ƒ (Dynamic)` へ変化**した。`cacheComponents` を有効化していない
構成では、レイアウトの動的APIがその配下すべてに伝播するため。

Vercel Free Plan の帯域（100GB/月）とサーバーレス関数の制約、および
「ISR/SSGを積極的に活用する」という方針に反するので採用しない。
**ルートレイアウトで `headers()` / `cookies()` / `searchParams` を使わないこと。**
...
`lang` はどの要素にも指定でき、支援技術は要素レベルの宣言を尊重する。
ヘッダー・フッターのナビゲーションは多言語ページでも日本語のままなので、
文書全体を `lang="en"` にするより**この範囲指定のほうが実態に即している**。

`HtmlLangSync` は `usePathname()` と `splitLocalePrefix()` からロケールを導くため、
クライアントサイド遷移で多言語ページを離れると `ja` へ戻る。
