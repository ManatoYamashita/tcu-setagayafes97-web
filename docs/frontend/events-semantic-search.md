...
> 誤爆をゼロに寄せている。**費用の最終的な歯止めはレート制限ではなく、プリペイド $5 という契約形態**
> （下の「費用の上限は…」を参照）。レート制限とゲートと CDN は、その $5 を長持ちさせるためにある。
...
### 費用の上限は、コンソールの設定ではなく契約形態で付いている

**TypeSafe はプリペイドの $5 のみで、クレジットカードを登録していない**（2026-09-21 時点）。
したがって**請求が暴走することは原理的に起きない。** 最悪でも $5 を使い切って終わる。

**使い切ったときに何が起きるか。** TypeSafe への問い合わせが失敗し、`/api/search` は 502 を返す。
クライアントは失敗として扱い、**段3までのリテラル検索の結果を出したまま静かに戻る**
（5種の失敗を注入して実測済み。上の「弾かれたとき、来場者にどう見えるか」を参照）。
**検索が壊れるのではなく、意味検索だけが消える。** 来場者には通知も出ない。

$5 は約 7,700 クエリに相当する（1クエリ $0.000649）。**残高を増やしたり
クレジットカードを登録したりすると、この安全装置は外れる。** そのときは上限を別途用意すること。

> [!NOTE]
> **コンソールに使用量上限の設定があるかは未確認。** 公式ドキュメントに記載が無く、
> API からも読み書きできない（2026-09-21 実測。`GET /v1/models` は 200 だが、
> `/v1/usage` `/v1/account` `/v1/limits` `/v1/billing` `/v1/keys` `/v1/organization` はすべて 404）。
> 残高を見るには [console.typesafe.ai](https://console.typesafe.ai) の Usage を人が開くしかない。
> **プリペイドで止まる以上、いまは急がない。**
