...
| 対象                   | 変更前            | 変更後              |
| ---------------------- | ----------------- | ------------------- |
| オープナー合計         | 2.40s             | **1.12s**（-53.3%） |
| うち空待機             | 0.40s（0.2s × 2） | 0s（削除）          |
| スライドアウト         | 1.20s             | 0.55s               |
| ヒーロー入場の出そろい | 0.94s             | **0.66s**           |
| デスクトップ全完了     | 2.40s             | **1.18s**           |
| セーフティタイムアウト | 6000ms            | 3120ms              |
| ページ側フェイルセーフ | 5000ms            | 2520ms              |

尺は GSAP に実際にタイムラインを組ませて実測し、`tl.duration()` が 1.12s、
`tl.labels.heroCue` が 0.52s であることを確認している。
