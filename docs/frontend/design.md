...
### 「実配信HEX」は2つある。配信CSSを grep して取れるほうは誰も見ていない

Lightning CSS は色ごとに **`#hex` と `lab()` の2本**を出力する。前者は
**`lab()` 非対応ブラウザ向けのフォールバック**であり、後者が実際に描画される値である。

> [!IMPORTANT]
> **一次定義は `oklch()` だが、出力CSSに `oklch()` は1つも残らない。**
> 実測（2026-09-19 / 本番）で `oklch` は **0 件**、`lab(` は 72 件、分岐は
> `@supports (color:lab(0% 0 0))` だった。**どちらが描画されるかを決めているのは
> `lab()` への対応であって、`oklch()` への対応ではない。**
> 確かめ直すときに見る対応表を間違えないこと。

**本プロジェクトの動作下限（Safari 16.4 / Chrome 111 / Firefox 128）は全て `lab()` 対応**なので、
フォールバックの `#hex` は**サポート対象の来場者には一度も描画されない。**

自前トークン（`primary-*` / `gray-*` / `secondary`）は sRGB ガマット内にあるため
2本の値が一致し、この区別は表に出ない。**食い違うのは Tailwind 既定パレットの
彩度が高い段だけである。** 高彩度の oklch は sRGB の外にあり、フォールバックは
CSS Color 4 の「彩度を落とす」ガマットマッピングで、ブラウザは別の詰め方で描画する。

実測（2026-09-19 / 本番 / Chrome / canvas の実ピクセル）:

| 色                    | 配信CSSの `#hex`（未使用） | 実際に描画される色 | `#070707` との比         |
| --------------------- | -------------------------- | ------------------ | ------------------------ |
| `green-500`           | `#00c758`                  | **`#00c950`**      | **9.08**                 |
| `blue-500`            | `#3080ff`                  | **`#2b7fff`**      | **5.35**                 |
| `orange-500`          | `#fe6e00`                  | **`#ff6900`**      | **6.98**                 |
| `amber-500`           | `#f99c00`                  | **`#fe9a00`**      | **9.44**                 |
| `purple-500`          | `#ac4bff`                  | **`#ad46ff`**      | 3.78（`#f5f5f5` 上）     |
| `primary-400`（自前） | `#bf73e3`                  | `#bf73e3`          | 一致するので較正に使える |

> [!WARNING]
> **`grep -o -- '--color-X:#......'` で取った値でコントラストを計算してはいけない。**
> 自前トークンだけなら正しい答えが出るが、Tailwind 既定パレットでは**フォールバック＝
> 誰も見ていない色**の比が出る。差は 0.06〜0.15 と小さく、**一見もっともらしい表になる**のが厄介である。
> 2026-09-19 の #227 で、この2系統の数値が同一PR内に別々の表として同居した。

**How to apply:** 既定パレットの比を出すときは配信CSSを読まず、**実ブラウザで
`getComputedStyle` の値を canvas に塗り、実ピクセルを読む。** 色空間の変換も
不透明度の合成もブラウザ自身にやらせられるので、1回で正しい sRGB が出る。
...
const toHex = (cssColor) => {
  cx.fillStyle = "#000";
  cx.fillRect(0, 0, 1, 1); // 下地を固定
  cx.fillStyle = cssColor;
  cx.fillRect(0, 0, 1, 1); // lab() / oklch() をそのまま渡せる
  const d = cx.getImageData(0, 0, 1, 1).data;
  return "#" + [d[0], d[1], d[2]].map((v) => v.toString(16).padStart(2, "0")).join("");
};
toHex(getComputedStyle(el).backgroundColor); // => "#00c950"
```

どちらを見ているか分からなくなったら、`primary-400` が `#bf73e3` になることを較正点にする。
...
#### コメントの中のクラス名 — ESLint の死角は別の検査が塞ぐ

**Tailwind のソース走査はテキスト走査であり、コードコメントの中のクラス名も
「使われている」と見なす。** 一方 ESLint の `no-restricted-syntax` が見るのは AST の
`Literal` と `TemplateElement` だけで、**コメントは対象外である。**
つまりガードの死角と Tailwind の感度がずれている。

2026-09-19 まで、`AboutHero.tsx` のコメントに説明として残していた
「旧グラデーションのクラス名」のせいで、**どこからも使っていない紫と桃の
ユーティリティ2本と対応する CSS 変数が、本番CSSへ出続けていた**（実測で確認）。
画面は壊れないが、消したはずの色が配信物に残る。

**この死角は [`scripts/assert-no-restricted-colors.mjs`](../../scripts/assert-no-restricted-colors.mjs)
が塞ぐ**（#230）。`Static Checks` から `pnpm check:colors` で走り、Tailwind の走査範囲の
**生テキスト**をそのまま見る。`git ls-files` 以外に何も要求しないので、同ジョブの
「`pnpm install` だけで完結する」性質も、**fork PR でも結果が出る**性質も壊さない。

|                                               | 見るもの                             | コメントが | 役割                        |
| --------------------------------------------- | ------------------------------------ | ---------- | --------------------------- |
| `eslint.config.mjs` の `no-restricted-syntax` | AST の `Literal` / `TemplateElement` | 見えない   | 禁止色を**書かない**        |
| `scripts/assert-no-restricted-colors.mjs`     | `src/` の生テキスト（追跡ファイル）  | **見える** | 禁止色が**配信CSSへ出ない** |

> [!IMPORTANT]
> **禁止リストの一次定義は [`scripts/restricted-color-tokens.mjs`](../../scripts/restricted-color-tokens.mjs)
> の1箇所で、上の2つが両方ともそこを読む。`@theme` へ段を足したら、そのファイルから外す。**
> 同じ正規表現を2箇所に置くと、片方だけ直して片方がすり抜ける。

落ちるのは**接頭辞を伴う形**だけである。色のユーティリティは必ず名前空間の接頭辞を持つため、
接頭辞の無い裸のトークンからは何も出力されない。**実測値を説明する表（「旧 `purple-500` は 3.78」）は
コメントに残せる。** 走査は接頭辞を列挙せずに判定するので、`hover:` や `md:` の変種も落ちる。

- **クラス名の形で書きたくなったら、接頭辞を外して書く。**
  「`from-purple-500`」ではなく「既定パレットの紫の500段」と書く
- 配信物そのものを確かめたいときは次を使う。**較正値が取れたことを先に確認する**のは、
  取得に失敗したときに「0 件＝異常なし」と読めてしまうのを防ぐためである（#230）

```bash
set -o pipefail
base=https://setagayafes.org
css=$(curl -fsS "$base/about" | grep -oE '/_next/static/chunks/[a-z0-9]+\.css' | head -1)
[ -n "$css" ] || { echo "CSSのURLを取得できませんでした"; exit 1; }
body=$(curl -fsS "$base$css") || exit 1
grep -q -- '--color-primary-400:#bf73e3' <<<"$body" \
  || { echo "較正値が無い。本体CSSではありません: $css"; exit 1; }
hits=$(grep -oE '[a-z-]+-(purple|violet|fuchsia)-[0-9]{2,3}' <<<"$body" | sort | uniq -c)
[ -n "$hits" ] && { echo "$hits"; exit 1; }
echo "残存ゼロ（本体CSS ${css}）"
```

**「何も出ない」を正常の合図にしない。** 残存ゼロなら最後の行がそう言い、
残っていれば内訳を出して終了コード 1 で終わる。取得に失敗したときも 1 で終わる。

> [!WARNING]
> **変数展開の直後に全角文字を置くときは `${css}` と書く。** `"$css）"` と書くと、
> bash が `）`（U+FF09）の先頭バイトまで変数名として飲み込み、**パスが消えて文字化けする**
> （macOS の bash 3.2 で実測。`en_US.UTF-8`）。出力が壊れるだけで終了コードは 0 のままなので、
> **「動いているように見えて壊れている」形になる。** この検査自体が防ごうとしている形と同じである。

退行を注入して赤くなることを実測した（2026-09-19）。

| 退行させた内容                                                | `pnpm check:colors`                  | `pnpm lint`      |
| ------------------------------------------------------------- | ------------------------------------ | ---------------- |
| コメントへクラス名を戻す（#230 が直した形）                   | **落ちる**                           | **通る**（死角） |
| `className` へ禁止色を書く                                    | 落ちる                               | 落ちる           |
| コメントへ**接頭辞なし**で書く（「旧 `purple-500` は 3.78」） | 通る（意図どおり）                   | 通る             |
| `globals.css` から `source(none)` を外す                      | **落ちる**（走査範囲を保証できない） | 通る             |
...
> [!WARNING]
> **本ファイルのカラースケール表とコントラスト表は、実際に配信される色（実配信値）で書いてある。** カラースケールは `@theme` で oklch 指定しており、Lightning CSS がビルド時に変換するため、デザイン仕様の HEX とは別の値が配信される。例: Secondary は仕様 `#E1C0EE` に対して実配信 `#d5a7ed`、ブランドカラーは仕様 `#CD79EE` に対して実配信 `#bf73e3`。**仕様 HEX が載っているのはブランドカラーの属性表だけで、そこには実配信 HEX を併記してある。**
>
> 上の表は**すべて自前トークンどうしの組み合わせ**で、いずれも sRGB ガマット内にあるため、次の grep で取れる値と実際の描画色が一致する。
>
> ```bash
> grep -o -- '--color-secondary:[^;]*' $(find .next -name '*.css' -path '*static*' | head -1)
> ```
>
> **ただし Tailwind 既定パレットには使えない。** 出力CSSには `#hex` と `lab()` の**2本**が出ており、この grep が拾うのは `oklch()` 非対応ブラウザ向けのフォールバックである。ガマット外の既定パレットでは両者が食い違う。詳細と実測は本ファイルの「カラーシステム > 『実配信HEX』は2つある」節を参照。
