...
| **primary-700 on Secondary（淡紫背景の文字色）** | **5.6:1**      | **✅ WCAG AA 適合（推奨）**      |
| primary-600 on Secondary（淡紫背景）             | 3.8:1          | ⚠️ 大テキスト・UI のみ           |
| primary-300 on Secondary（淡紫背景）             | 1.2:1          | ❌ 前景色として使用禁止          |
...
> - **`bg-white/10` は白いカードではない。** 淡紫のページ背景に白を 10% 重ねただけで、背景とほぼ同じ明度になる。カード内の文字色を「白背景向け」の基準で選ばないこと。

> [!WARNING]
> **上表および本ファイル内の HEX 値は、実際に配信される色とずれる。** カラースケールは `@theme` で oklch 指定しており、Lightning CSS がビルド時に sRGB へ変換する（出力CSSに oklch は残らない）。実測では Secondary が `#E1C0EE`（記載値）ではなく `#d5a7ed` で配信されていた。**コントラストを検証するときは、記載の HEX ではなく出力CSSの実値を使うこと。**
>
> ```bash
