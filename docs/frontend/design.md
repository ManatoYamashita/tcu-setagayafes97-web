...
**`oklch()` 非対応ブラウザ向けのフォールバック**であり、後者が実際に描画される値である。

**本プロジェクトの動作下限（Safari 16.4 / Chrome 111 / Firefox 128）は全て `oklch()` 対応**なので、
...
| `pink-*`               | `special` バッジの既定配色                             | 分類の識別。**5.62:1**                               |
...
- **禁止した色をコメントで説明するときは、クラス名を原形で書かない。**
  「`from-purple-500`」ではなく「既定パレットの紫の500段」と書く
- 疑わしいときは配信CSSを直接数える。0 でなければコメントかテンプレートに原形が残っている

```bash
curl -s "$(curl -s https://setagayafes.org/about \
  | grep -oE 'href="[^"]*\.css"' | head -1 | sed 's/href="//;s/"//' \
  | sed 's|^|https://setagayafes.org|')" \
  | grep -coE '\.(from|via|to|text|bg|border)-(purple|violet|fuchsia)-[0-9]{2,3}'
```
