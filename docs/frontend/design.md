...
> 15箇所が一斉に未達になる。「トークンに寄せる」置換ほど、**寄せ先の比を先に計算すること。**
...
| 色相                   | 残している用途                                 | 理由                                                 |
| ---------------------- | ---------------------------------------------- | ---------------------------------------------------- |
| `red-*`                | 緊急のお知らせ、フォームのエラー、危険物の注意 | **危険・エラーの意味**を担う。他の色では代替できない |
| `green-*`              | 教室企画のバッジ、ゴミの注意                   | 分類の識別。`room` バッジは 8.93:1                   |
| `blue-*`               | お知らせのバッジ、駐車場の注意                 | 分類の識別。`news` バッジは 5.43:1                   |
| `orange-*` / `amber-*` | ステージ・模擬店のバッジ、ペットの注意         | 分類の識別。7.14 / 9.38:1                            |
| `pink-*`               | 著名人企画のバッジ                             | 分類の識別。5.62:1                                   |

**基準は「意味の異なる分類を一目で区別する」かどうかである。**
装飾のグラデーションや、単に紫の代わりに使われている色は対象外で、`primary-*` へ寄せる。

**紫だけは分類用途でも使わない。** ブランドと衝突し、「押せる／ブランド」の意味が薄れるため。
注意事項カードの `photos` 分類が `purple-400` から `primary-400` になったのはこのため。
