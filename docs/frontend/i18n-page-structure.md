...
このとき、**リンクの href やラベルはブラウザ自動化で確認できますが、開閉アニメーションの再生は確認できません**（自動化タブは `visibilityState: "hidden"` で requestAnimationFrame が停止するため）。アニメーションの目視は実機で行ってください。詳細は [agent-browser-workflow.md の「検証できないもの」](./agent-browser-workflow.md#検証できないもの先に読むこと) を参照。
