 ...
 *
 * > [!IMPORTANT]
 * > **IP は来場者ひとりを指しません。** 学園祭当日は大学構内の Wi-Fi から数千人が
 * > 同じ出口IPで出てきます。1人あたりの妥当な回数（数回/分）をそのまま上限にすると、
 * > **来場者を自分たちで 429 で弾くことになります。**
 *
 * したがってここは「1人あたりの妥当な値」ではなく「1つの出口IPの背後にいる集団が
 * 到達しうる値」で置きます。費用の本丸はこの上限ではなく、
 *
 * - 段1〜3で当たったら呼ばない（大半のクエリが無料）
 * - CDN の `s-maxage=600`（同じ要望の再訪は関数へ届かない）
 * - TypeSafe コンソール側の使用量上限
 *
 * の3つで、レート制限は暴走を止める最後の柵という位置づけです。
 */
