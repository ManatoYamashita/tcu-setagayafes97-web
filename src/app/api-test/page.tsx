    ...
    <div className="container mx-auto px-4 py-16">
      ...
      <section className="mb-16">
        <h2 className="mb-6 text-2xl font-bold">
          お知らせ（getNewsList）
          <span className="ml-3 text-sm text-gray-900/60">取得件数: {newsList.length}</span>
        </h2>
        <div className="grid gap-6 md:grid-cols-3">
          {newsList.map((news) => (
            <div key={news.id} className="rounded-lg border p-4">
              ...
              <p className="text-sm text-gray-900/80">{news.description}</p>
              <div className="mt-3 text-xs text-gray-900/50">ID: {news.id}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-16">
        <h2 className="mb-6 text-2xl font-bold">
          企画一覧（getEventsList）
          <span className="ml-3 text-sm text-gray-900/60">取得件数: {eventsList.length}</span>
        </h2>
        <div className="grid gap-6 md:grid-cols-3">
          {eventsList.map((event) => (
            <div key={event.id} className="rounded-lg border p-4">
              ...
              <p className="mb-2 text-sm text-gray-900/60">主催: {event.organizer}</p>
              <p className="text-sm text-gray-900/80">{event.description}</p>
              <div className="mt-2 text-xs text-gray-900/50">
      ...
      <section className="mb-16">
        <h2 className="mb-6 text-2xl font-bold">
          ...
          <span className="ml-3 text-sm text-gray-900/60">取得件数: {featuredEvents.length}</span>
        </h2>
        <div className="grid gap-6 md:grid-cols-3">
          {featuredEvents.map((event) => (
            <div key={event.id} className="rounded-lg border-2 border-primary bg-secondary/5 p-4">
              <div className="mb-2 flex gap-2">
                <Badge variant={event.date} label={event.date} />
                <Badge variant={event.type} label={event.type} />
              </div>
              <h3 className="mb-1 font-semibold">{event.title}</h3>
              <p className="mb-2 text-sm text-gray-900/60">主催: {event.organizer}</p>
              <p className="text-sm text-gray-900/80">{event.description.slice(0, 80)}...</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-16">
        <h2 className="mb-6 text-2xl font-bold">
          ...
          <span className="ml-3 text-sm text-gray-900/60">取得件数: {sponsorsList.length}</span>
        </h2>
        <div className="grid gap-6 md:grid-cols-3">
