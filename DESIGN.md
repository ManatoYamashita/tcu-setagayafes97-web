...
一次定義は `src/app/globals.css` の `@theme` 1箇所だけで、**ソースへの `#CD79EE` 直書きは #143 で一掃した。** CSS が効かない3箇所（メールHTML・WebGL シェーダ・メタデータ文字列）のみ実配信 HEX を直書きしており、`@theme` を変更したら手で追従させる必要がある。一覧は `docs/frontend/design.md`「ブランドカラー」を参照。
...
> [!NOTE]
> **上の値は淡紫のページ背景（`bg-secondary`）を前提にした `src/components/ui/Card.tsx` の
> 現状をそのまま書き写したものである。** `PageSheetLayout` の白いシート（`bg-white`）の上では
> `default` の枠は 1.08:1 になって消える。白いシート上のカードは §9 の指針に従うこと。
...
| Pattern              | 用途                                     | 白いシート上の対応値       |
| -------------------- | ---------------------------------------- | -------------------------- |
| `bg-white/10`        | カード背景（**淡紫背景のページ限定**）   | `bg-white` / `bg-gray-50`  |
| `bg-white/20`        | セカンダリボタン（淡紫背景のページ限定） | 背景に応じて不透明色       |
| `bg-black/60`        | オーバーレイテキスト背景                 | 同じ（画像の上なので共通） |
| `border-gray-200/20` | 薄いボーダー（**淡紫背景のページ限定**） | `border-gray-200`          |

> [!NOTE]
> 透過値は下地の色で見え方が決まる。**白いシート（`PageSheetLayout`）の上では
> `bg-white/10` は純白、`border-gray-200/20` は 1.08:1 になり、どちらも消える。**
> 白いシート上の値の決め方は §9 を参照。
...
| -------------------------------- | ------------------------------------------------ | -------------------- |
| カード・コンテナの面             | `bg-white`                                       | —                    |
| 面を一段沈めたい枠（空状態など） | `bg-gray-50`                                     | —                    |
| カード・コンテナの枠             | `border-gray-200`                                | 1.53:1               |
| カードの識別アクセント           | `border-l-4 border-l-primary-600`                | 7.45:1               |
| **情報を伝える線**（グリッド等） | `border-gray-400`                                | **3.23:1**           |
| 選択中のタブ・トグル             | `bg-primary-600 text-white`                      | 7.45:1               |
| 未選択のタブ・トグル             | `border-gray-200 bg-white hover:border-gray-400` | 1.53:1 / hover 3.23  |

> [!IMPORTANT]
> **線が「装飾」か「情報」かで基準が変わる。** カードの外枠は装飾なので `border-gray-200`
> （1.53:1）で足りるが、**タイムテーブルのグリッド線のように位置や量を読み取らせる線は
> WCAG 1.4.11（非テキストコントラスト）の 3:1 が要る。** `border-gray-200` では届かないため
...
> [!WARNING]
> **このパターンは淡紫のページ背景（`bg-secondary`）専用である。**
> `PageSheetLayout` の白いシート（`bg-white`）の上では、白 10% は純白に、
> `border-gray-200/20` は 1.08:1 になり、ガラス感どころか要素の輪郭ごと失われる。
> 白いシート上は §9 の不透明値を使うこと。
